...
/* USER CODE BEGIN PFP */
uint8_t W25Q16_ReadStatus(void);
/* USER CODE END PFP */
...
{
  W25Q16_WriteEnable();
  uint8_t cmd[] = {0x02, (uint8_t)(address >> 16), (uint8_t)(address >> 8), (uint8_t)(address)};
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi1, cmd, sizeof(cmd), 100);
  HAL_SPI_Transmit(&hspi1, &data, sizeof(data), 100);
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_SET);
  while ((W25Q16_ReadStatus() & 0x01) == 0x01);
  printf("writing completed\r\n");
}

uint8_t W25Q16_ReadByte(uint32_t address)
{
  uint8_t cmd[] = {0x03, (uint8_t)(address >> 16), (uint8_t)(address >> 8), (uint8_t)(address)};
  uint8_t data;
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_RESET);
...
uint8_t W25Q16_ReadStatus(void)
{
  ...
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi1, &cmd, sizeof(cmd), HAL_MAX_DELAY);
  HAL_SPI_Receive(&hspi1, &status, sizeof(status), HAL_MAX_DELAY);
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_SET);
  return status;
}
...
{
  W25Q16_WriteEnable();
  uint8_t cmd[] = {0x20, (uint8_t)(address >> 16), (uint8_t)(address >> 8), (uint8_t)(address)};
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi1, cmd, sizeof(cmd), HAL_MAX_DELAY);
  HAL_GPIO_WritePin(W25Q16_CS_GPIO_Port, W25Q16_CS_Pin, GPIO_PIN_SET);
  while ((W25Q16_ReadStatus() & 0x01) == 0x01);
  printf("sector is erased\r\n");
}
...
int main(void)
{
  ...
  //W25qxx_Init();
  W25Q16_WriteEnable();
  W25Q16_EraseSector(0x00);
  W25Q16_WriteByte(0x00,250);
  uint8_t new=00;
  new=W25Q16_ReadByte(0x00);
  printf("%d\n",new);
  /* USER CODE END 2 */
